...
DCEL monotoneTriangulation(DCEL &dcel) {
  ...
  for (size_t i = 1; i < new_dcel.faces.size(); ++i) {
    ...
    // Traverse the half-edges of the face to collect vertices in the correct
    // order
    do {
    ...
    if (j == 3) {
      if (!(triangle[0].x == triangle[1].x && triangle[0].y == triangle[1].y) &&
        ...
        std::cerr << "Skipping degenerate triangle: (" << triangle[0].x << ","
  ...
  if (out_triangle.is_open()) {
    ...
    std::cerr << "Error: Unable to open file " << TRIANGLE_EXPORT_FILE
              << std::endl;
  }
...
void faceTriangulation(Face &face, DCEL &dcel,
  ...
  for (int j = 2; j < vertices.size() - 1; j++) {
    ...
    if (dcel.vertices[ji].point.isBelow(dcel.vertices[jpi].point) !=
      ...
      while (!st.empty()) {
        if (dcel.vertices[ji].point.isBelow(dcel.vertices[jpi].point)) {
          if (!counterClockwise(vertices[j].first->point, lpop.first->point,
                                st.top().first->point)) {
            diagonals.push_back(
